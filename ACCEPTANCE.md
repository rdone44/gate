...
- `test/evaluator.test.mjs` — unit tests for evaluator/report/CLI (71 tests, incl. push-to-main fixture introduced in commit 761b161)
...
| Five deterministic rules                                 | PASS    | Covered by `test/evaluator.test.mjs` + `test/collector.test.mjs` — 91/91 pass (incl. pr-merged, push-to-main fixture, whitespace title rejection). |
...
| Automated tests                                          | PASS    | Real `npm test` run: 91/91 passed (71 evaluator + 20 collector), 2.80s. |
...
- **GitHub collector mode shipped (v0.3.0–v0.5.3).** `src/collector.mjs` and `bin/gate.mjs collect` are now implemented and tested (91/91 pass including §16.10 stubbed-fetch integration tests). The previous gap — CLI only performing offline evaluation — is resolved. Commit `761b161` adds push-to-main fixture (pr-merged FAIL when no pr field). Commit `d5ec4e1` adds whitespace-only task.title rejection (v0.5.3).
